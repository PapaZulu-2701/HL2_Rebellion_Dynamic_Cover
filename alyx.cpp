...
CNPC_Alyx::CNPC_Alyx()
{
	...
	m_hLowCoverProp = NULL;				// [MODIFICATION] Handle to selected low cover prop for dynamic cover system.
	m_hHighCoverProp = NULL;			// [MODIFICATION] Handle to selected high cover prop for dynamic cover system.
...
// ============================================
int CNPC_Alyx::CountNPCsUsingProp(CBaseEntity* pProp)
{
	if (!pProp)
		return 0;
	...
	for (CEntitySphereQuery sphere(pProp->GetAbsOrigin(), 600.0f); (pEntity = sphere.GetCurrentEntity()) != NULL; sphere.NextEntity())
	{
		...
		if (pAlyx->m_hLowCoverProp == pProp || pAlyx->m_hHighCoverProp == pProp)
		{
			count++;
		}
	}
...
bool CNPC_Alyx::DynamicPropCover()
{
...
void CNPC_Alyx::GatherConditions()
{
	...
	if (GetState() == NPC_STATE_COMBAT && GetEnemy())
	{
		...
		{
			ClearCondition(COND_ALYX_HAS_LOW_COVER);
			ClearCondition(COND_ALYX_HAS_HIGH_COVER);
...
int CNPC_Alyx::SelectSchedule( void )
{
	...
	{
		bool hasLow = HasCondition(COND_ALYX_HAS_LOW_COVER);
		bool hasHigh = HasCondition(COND_ALYX_HAS_HIGH_COVER);
		...
		if (hasLow && hasHigh)
		{
			...
			if (lowValid && highValid)
			{
				...
				return useLowCover ? SCHED_ALYX_MOVE_TO_LOW_COVER : SCHED_ALYX_MOVE_TO_HIGH_COVER;
			}
			else if (lowValid)
			{
				Msg("[DEBUG] SelectSchedule: Only LOW COVER in valid range\n");
				return SCHED_ALYX_MOVE_TO_LOW_COVER;
			}
			else if (highValid)
			{
				Msg("[DEBUG] SelectSchedule: Only HIGH COVER in valid range\n");
				return SCHED_ALYX_MOVE_TO_HIGH_COVER;
			}
			else
			{
...
void CNPC_Alyx::StartTask( const Task_t *pTask )
{
	switch( pTask->iTask )
	{
	case TASK_ALYX_MOVE_TO_HIGH_COVER:  // [MODIFICATION-Dynamic Cover]
	{
	...
	case TASK_ALYX_MOVE_TO_LOW_COVER: // [MODIFICATION-Dynamic Cover]
	{
...
AI_BEGIN_CUSTOM_NPC( npc_alyx, CNPC_Alyx )
	...
	DECLARE_TASK(TASK_ALYX_MOVE_TO_LOW_COVER)//[MODIFICATION] - Dynamic Cover
	DECLARE_TASK(TASK_ALYX_MOVE_TO_HIGH_COVER)  //[MODIFICATION] - Dynamic Cover
	...
	DECLARE_CONDITION(COND_ALYX_HAS_HIGH_COVER) //[MODIFICATION] - Dynamic Cover
	DECLARE_CONDITION(COND_ALYX_HAS_LOW_COVER) //[MODIFICATION] - Dynamic Cover
	...
	DEFINE_SCHEDULE
		...
		//=========================================================
		// SCHED_ALYX_MOVE_TO_HIGH_COVER //[MODIFICATION] - Dynamic Cover
		//=========================================================
		DEFINE_SCHEDULE
		(
			SCHED_ALYX_MOVE_TO_HIGH_COVER,

			"	Tasks"
			"		TASK_STOP_MOVING						0"
			"		TASK_ALYX_MOVE_TO_HIGH_COVER			0"
...
//=========================================================
// SCHED_ALYX_MOVE_TO_LOW_COVER //[MODIFICATION] - Dynamic Cover
//=========================================================

		DEFINE_SCHEDULE
		(
	SCHED_ALYX_MOVE_TO_LOW_COVER,

			"   Tasks"
			"		TASK_STOP_MOVING					    0"
			"		TASK_ALYX_MOVE_TO_LOW_COVER				0"
			"		TASK_RUN_PATH_TIMED						4"
			"		TASK_WAIT_FOR_MOVEMENT					0"
			"		TASK_FACE_ENEMY							0"
			"		TASK_RANGE_ATTACK1						0"
